<template>
  <div class="app d-flex" style="background-color: #4F4752; min-height: 100vh; color: #373239">
    <div style="width: 180px" class="d-flex flex-column align-items-center position-fixed">
      <div class="app-icon mt-5" style="width: 76px; height: 76px"></div>
      <div>
        <div
          class="mt-4 py-2 mx-3 c-pointer menu-tab border-bottom"
          :class="{ active: $router.currentRoute.value.path == `/` }"
          @click="$router.push('/')"
        >
          <div>Проекты</div>
        </div>
        <div
          v-show="projectId"
          class="py-2 mx-3 c-pointer menu-tab border-bottom"
          :class="{ active: $router.currentRoute.value.path == `/project/${projectId}` }"
          @click="$router.push(`/project/${projectId}`)"
        >
          <div>Планирование</div>
        </div>
        <div
          v-show="projectId"
          class="py-2 mx-3 c-pointer menu-tab border-bottom"
          :class="{ active: $router.currentRoute.value.path == `/project/${projectId}/kanban` }"
          @click="$router.push(`/project/${projectId}/kanban`)"
        >
          <div>Выполнение</div>
        </div>
        <div
          v-show="projectId"
          class="py-2 mx-3 c-pointer menu-tab"
          :class="{ active: $router.currentRoute.value.path == `/project/${projectId}/dashboard` }"
          @click="$router.push(`/project/${projectId}/dashboard`)"
        >
          <div>Итоги</div>
        </div>
      </div>
    </div>
    <div
      class="p-3"
      style="
        margin-left: 180px;
        flex: 1;
        background-color: #373239;
        border-radius: 20px 0 0 20px;
        box-shadow: -8px 0 12px #00000029;
        overflow: hidden;
      "
    >
      <div class="row">
        <div class="col-12">
          <slot></slot>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent } from "vue";

interface Data {}

export default defineComponent({
  components: {},
  data() {
    return {} as Data;
  },
  methods: {},
  computed: {
    projectId() {
      if (this.$route.params.id) {
        var id = this.$route.params.id as string;
        return id;
      }

      return null;
    },
  },
  watch: {},
  async mounted() {},
});
</script>

<style>
.app-icon {
  background-image: url("data:image/svg+xml,%3Csvg width='76' height='76' viewBox='0 0 76 76' fill='none' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3E%3Crect x='21' y='16' width='37' height='12' fill='url(%23pattern0_44_108)'/%3E%3Crect x='21' y='42' width='27' height='12' fill='url(%23pattern1_44_108)'/%3E%3Cpath d='M33.0238 41.1667V53.8333M45.6904 15.8333V28.5M7.69043 9.5V60.1667C7.69043 61.8464 8.35769 63.4573 9.54542 64.645C10.7332 65.8327 12.3441 66.5 14.0238 66.5H64.6904M23.5238 41.1667H45.6904C47.4393 41.1667 48.8571 42.5844 48.8571 44.3333V50.6667C48.8571 52.4156 47.4393 53.8333 45.6904 53.8333H23.5238C21.7749 53.8333 20.3571 52.4156 20.3571 50.6667V44.3333C20.3571 42.5844 21.7749 41.1667 23.5238 41.1667ZM23.5238 15.8333H55.1904C56.9393 15.8333 58.3571 17.2511 58.3571 19V25.3333C58.3571 27.0822 56.9393 28.5 55.1904 28.5H23.5238C21.7749 28.5 20.3571 27.0822 20.3571 25.3333V19C20.3571 17.2511 21.7749 15.8333 23.5238 15.8333Z' stroke='white' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/%3E%3Cdefs%3E%3Cpattern id='pattern0_44_108' patternContentUnits='objectBoundingBox' width='0.432432' height='0.666667'%3E%3Cuse xlink:href='%23image0_44_108' transform='scale(0.00108108 0.00333333)'/%3E%3C/pattern%3E%3Cpattern id='pattern1_44_108' patternContentUnits='objectBoundingBox' width='0.592593' height='0.666667'%3E%3Cuse xlink:href='%23image0_44_108' transform='scale(0.00148148 0.00333333)'/%3E%3C/pattern%3E%3Cimage id='image0_44_108' width='400' height='200' xlink:href='data:image/png;base64,'/%3E%3C/defs%3E%3C/svg%3E%0A");
}
.menu-tab {
  border-bottom-color: #AEA4B6 !important;
  color: #fff;
  font-size: 20px;
}
.menu-tab.active {
  color: #D66434;
  width: fit-content;
}
</style>
